...
// glasses under the top glass will remain empty. Example 2:
...
// liquid equally, and each will get half cup of champange. Example 3:
...
public:
  double champagneTower(int poured, int query_row, int query_glass) {
    // Create a 2D vector to represent the champagne tower with 'query_row + 1'
    // rows. Each row has 'query_row + 1' glasses, initialized with 0.0
    // champagne.
    ...
    // Iterate through each row of the tower.
    for (int row = 0; row < query_row; ++row) {
      // Iterate through each glass in the current row.
      for (int glass = 0; glass <= row; ++glass) {
        // Calculate the overflow of champagne from the current glass.
        double overflow = (tower[row][glass] - 1.0) / 2.0;
        if (overflow > 0) {
          // Distribute the overflow evenly to the glasses in the next row.
